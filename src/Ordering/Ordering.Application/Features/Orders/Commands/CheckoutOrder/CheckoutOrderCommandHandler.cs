...
namespace Ordering.Application.Features.Orders.Commands.CheckoutOrder
{
	public class CheckoutOrderCommandHandler : IRequestHandler<CheckoutOrderCommand, int>
	{
        ...
        private async Task SendEmail(Order order, CancellationToken cancellationToken)
        {
            try
            {
                await _emailService.SendEmailAsync(new()
                {
                    To = "lguilherme.pcarvalho@gmail.com",
            ...
            {
                _logger.LogError(ex, "Order {orderId} failed due to an error with the mail service: {errorMessage}", order.Id, ex.Message);
            }
        }
    }
}
